...
export default class Slider extends React.Component<any, any> {
  ...
  setSlide(i: number) {
    ...
    } else if (this.currentIndex === 0 && i ===  (this.props.slides.length - 1)) {
      this.fromRight = true;
    }
    // if (i === 0 ) {
    //   this.fromRight = false;
    ...
    let lefthiddenIndex = (i === 0) ? this.countImg - 2 : (i === 1) ? this.countImg - 1 : i - 2;
    this.slideData = this.props.slides.map((value, index) => {
      let slide = {
        index: index,
        src: value.source,
        ...
        state:
          index === this.currentIndex
            ? ('centerSlide z3' + (this.fromRight ? ' fromLeft' : ' fromRight'))
            : index === rightIndex
  ...
  render() {
    let Component = this.slideData.map((value, index) => {
      if (value.state) {
        return (
          <div key={index} className={`slideForm ${value.state}`} onClick={() => this.setSlide(index)} >
            <img src={value.src} />
          </div>
        );
      }
    ...
    this.togl = this.props.slides.map((value, index) => {
        return (
          <Togl key={index} active={index === this.currentIndex} onClick={() => this.setSlide(index)}><hr/></Togl>
        );
    });
